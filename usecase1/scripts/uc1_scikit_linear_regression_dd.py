...
def graph_data():
    ...
    try:
        ...
        #data3_d = data3_d.rename(columns={'defect_count/KLOC': 'defect_density'})
            graph_data = data5_d[:]
    finally:
        cnx.close()
    print(ucl1,lcl1)
    predicted_result=predict_dd()
...
def predict_ucllcl(mydata):
    ...
    three_sigma=3*sigma
    print(three_sigma)
    print type(average),type(three_sigma)
...
def last_prediction():
   ...
   strColumns = ','.join(cols)
   query = "select " + strColumns + " from uc_dd_last_prediction where Algo='LINEAR'"
   try:
      cursor = cnx.cursor()
      cursor.execute(query)
      #data = DataFrame(cursor.fetchall(), columns=cols)
